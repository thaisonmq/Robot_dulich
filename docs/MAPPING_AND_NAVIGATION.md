...
4. Mặc định mọi lần map load, mở Control, mất định vị và nhập vị trí gần đúng đều định vị **thụ động**, chỉ dùng LiDAR/TF tại chỗ và không phát velocity. Khi đã phải chuyển sang global search, UI hiện ngay **Cho phép xoay để định vị** và **Chỉ vị trí robot gần đúng**, không bắt người vận hành chờ hết timeout. `LOCALIZING_ROTATING` chỉ được phép khi lệnh riêng truyền `allow_rotation=true`; ngưỡng quay tại chỗ cho phép tia quét lệch tối đa 7 cm vào vòng quét góc của footprint bảo thủ 0,40 × 0,36 m, nhưng vẫn chặn vật thể chạm sát cạnh thân. Các tia LiDAR có endpoint nằm bên trong chính footprint được loại như self-return trước AMCL/costmap; tia bên ngoài không bị che. Velocity vẫn đi qua `/cmd_vel_nav -> twist_mux -> velocity smoother -> motion-safety`, và E-stop, scan stale, obstacle/directional mask hoặc manual control dừng xoay ngay.

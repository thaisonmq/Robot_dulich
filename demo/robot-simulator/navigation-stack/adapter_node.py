...
class NavigationAdapter(Node):
    def __init__(self) -> None:
        ...
        self.last_sensor_time_resume_attempt_monotonic = 0.0
        ...
        self.dynamic_local_bypass_minimum_deviation = configured(
        ...
        self.corridor_hard_side_margin = configured(
            "corridor_hard_side_margin", 0.04
        )
        ...
        self.stop_turn_minimum_route_side_clearance = configured(
            "stop_turn_minimum_route_side_clearance", 0.04
        )
        ...
        self.dynamic_route_selection_timeout = max(
            1.0,
            configured("dynamic_route_selection_timeout_seconds", 10.0),
        )
        ...
        self.stop_turn_exact_retry_planning_budget = configured(
            "stop_turn_exact_retry_planning_budget_seconds", 6.0
        )
        self.stop_turn_exact_retry_minimum_translation = configured(
            "stop_turn_exact_retry_minimum_translation_m", 0.005
        )
        self.stop_turn_exact_retry_minimum_yaw = math.radians(configured(
            "stop_turn_exact_retry_minimum_yaw_degrees", 0.5
        ))
        ...
        self.blocked_goal_standoff_max_distance = max(
            self.stop_turn_final_position_tolerance,
            configured("blocked_goal_standoff_max_distance", 0.22),
        )
        self.blocked_goal_standoff_confirmation = configured(
            "blocked_goal_standoff_confirmation_seconds", 0.75
        )
        self.blocked_goal_standoff_minimum_clearance = configured(
            "blocked_goal_standoff_minimum_front_clearance", 0.04
        )
        ...
        self.dynamic_route_selection_deadline_monotonic = 0.0
        ...
        self.straight_steering_authority_lost_since: float | None = None
        self.straight_steering_authority_recovery_active = False
        ...
        self.straight_steering_authority_timeout = configured(
            "straight_steering_authority_timeout_seconds", 0.40
        )
        ...
        self.straight_hard_cross_track = configured(
            "straight_hard_cross_track", 0.05
        )
            ...
            if active is None or not pose_fresh:
                ...
                if not decision.forward_allowed:
                    ...
                    steering_blocked = bool(
                        self._atomic_safety_fresh(now)
                        and steering_correction_blocked(
                            requested_angular,
                            self.safety_direction_mask,
                        )
                    )
                    if steering_blocked:
                        # Do not keep translating after motion-safety has
                        # removed the correction that holds this centerline.
                        # Confirm while stationary, then move along the
                        # current chassis heading to a proven steering bay.
                        linear = 0.0
                        angular = 0.0
                        reasons = tuple(reasons) + (
                            "STEERING_SAFETY_STOP",
                        )
                        if self.straight_steering_authority_lost_since is None:
                            self.straight_steering_authority_lost_since = now
                            self._nav_debug(
                                "STRAIGHT_RECOVERY",
                                reason="STEERING_DIRECTION_BLOCKED",
                                action="STOP_AND_CONFIRM",
                                requested_linear=requested_linear,
                                requested_angular=requested_angular,
                                direction_mask=self.safety_direction_mask,
                            )
                        elif (
                            not self.straight_steering_authority_recovery_active
                            and now - self.straight_steering_authority_lost_since
                            >= self.straight_steering_authority_timeout
                        ):
                            self._recover_lost_straight_steering(
                                (requested_linear, requested_angular, now),
                                self.pipeline_samples.get("motion_safety"),
                            )
                    elif not self.straight_steering_authority_recovery_active:
                        self.straight_steering_authority_lost_since = None
    ...
    def _record_pipeline(
        ...
        *,
        ...
        # These bounded latest-value samples are part of the runtime safety
        # contract, not merely diagnostics. In particular the adapter must
        # detect when Motion Safety preserves translation while clipping away
        # the steering correction needed to stay on a straight segment.
    ...
    def _dispatch(self, command: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if command == "navigation.alternatives":
            self._validate_command_map(payload)
            # A preview is motionless and may be rehydrated by a fresh browser
            # before alternatives are requested. Rebind the edge state to the
            # Center mission carried by every mission action.
            self.current_mission_id = str(
                payload.get("mission_id") or self.current_mission_id
            )
        ...
        if command == "navigation.route_selection_back":
            ...
            if target == "WAITING_FOR_DYNAMIC_CLEAR":
                # Older Center clients called this "keep waiting for the old
                # route". Bounded recovery no longer has an indefinite-wait
                # branch, so treat that legacy action as the user's cancel.
    ...
    def _route_metadata(
        ...
        *,
        ...
        if self.saved_map is None or self.map_navigation_geometry is None:
            ...
            static_validation = self._validate_static_stop_turn_route(
                canonical,
                directions,
                initial_reverse_clearance_recovery=(
                    initial_reverse_clearance_recovery
                ),
            )
    ...
    def _validate_static_stop_turn_route(
        self,
        points: list[dict[str, float]],
        segment_directions: list[int] | tuple[int, ...],
        *,
        ...
        """Revalidate a route without erasing its bounded bay escape.

        The planner may authorize exactly one short initial reverse segment at
        physical-body clearance when the measured pose is already inside the
        request-local reserve. Every segment from the bay onward, and the bay
        turn itself, still uses the normal translation reserve here. Raw API
        paths cannot opt into this mode; the flag only comes from a route that
        the exact planner has already proved.
        """
        if self.saved_map is None:
            return ExecutablePathValidation(False, "MAP_MISSING")
        canonical = canonicalize_stop_turn_path(points)
        directions = tuple(
        ...
        if not initial_reverse_clearance_recovery:
            return validate_stop_turn_route(
                self.saved_map,
                canonical,
                half_length=self.footprint_half_length,
        ...
        if (
            len(canonical) < 3
            or len(directions) != len(canonical) - 1
            or directions[0] >= 0
            or any(direction < 0 for direction in directions[1:])
            or math.hypot(
                float(canonical[1]["x"]) - float(canonical[0]["x"]),
                float(canonical[1]["y"]) - float(canonical[0]["y"]),
            ) > self.turn_bay_max_distance + 1e-9
        ):
            return ExecutablePathValidation(
                False, "INITIAL_REVERSE_CLEARANCE_RECOVERY_INVALID"
            )
        initial = validate_stop_turn_route(
            self.saved_map,
            canonical[:2],
            half_length=self.footprint_half_length,
            half_width=self.footprint_half_width,
        ...
        if not initial.valid:
            return initial
        suffix = validate_stop_turn_route(
            self.saved_map,
            canonical[1:],
            half_length=self.footprint_half_length,
            half_width=reserved_half_width,
        ...
        outgoing_yaw = math.atan2(
            float(canonical[2]["y"]) - float(canonical[1]["y"]),
            float(canonical[2]["x"]) - float(canonical[1]["x"]),
        )
        bay_turn = validate_rotation_sweep(
            self.saved_map,
            float(canonical[1]["x"]),
            float(canonical[1]["y"]),
            incoming_yaw,
        ...
        if not bay_turn.valid:
            return ExecutablePathValidation(
                ...
                float(canonical[1]["x"]),
                float(canonical[1]["y"]),
                bay_turn.collision_yaw,
                samples_checked=(
                    initial.samples_checked
                    + suffix.samples_checked
                    + bay_turn.samples_checked
                ),
            )
        return ExecutablePathValidation(
            True,
    ...
    def _serialize_stop_turn_candidates(
        ...
        for planned_route in planned:
            ...
            initial_reverse_clearance_recovery = bool(
                planned_route.initial_reverse_clearance_recovery
            )
            directions = list(
    ...
    def _validate_executable_path(
        ...
        *,
        ...
        live_validation = validate_executable_grid_path(
            ...
            # A start escape deliberately begins inside a Saved Map raster
            # disagreement. StaticLayer inflation can therefore put cost 99
            # under the centerline even while the exact physical footprint is
            # moving safely away from the wall. In this one bounded mode,
            # validate real lethal cells with the full body and let the fresh
            # directional safety snapshot cover live obstacles. Normal routes
            # retain the stricter center-on-inscribed rejection.
            inscribed_threshold=(
                None if allow_monotonic_initial_overlap else 99
            ),
            allow_monotonic_initial_overlap=allow_monotonic_initial_overlap,
            maximum_new_initial_overlap_cells=(
                maximum_new_initial_overlap_cells
            ),
        )
    ...
    def _safety_status_callback(self, message: String) -> None:
        ...
        if (
            dynamic_stop
            and bool(snapshot.get("hard_stop"))
            and self.current_state == "NAVIGATING"
            and self.execution_phase in {
            ...
            self._nav_debug(
                "TURN_RECOVERY",
                action="RELOCATE_AFTER_HARD_STOP",
        ...
        now = time.monotonic()
        profile = self.pipeline_samples.get("auto_profile")
        final = self.pipeline_samples.get("motion_safety")
        steering_clipped = bool(
            self.current_state == "NAVIGATING"
            and self.execution_phase in {"STRAIGHT", "NARROW_STRAIGHT"}
            and profile is not None
            and final is not None
            and now - profile[2] <= 0.35
            and now - final[2] <= 0.35
            and abs(profile[0]) >= 0.02
            and abs(profile[1]) >= 0.03
            and abs(final[0]) >= 0.02
            and (
                abs(final[1]) < 0.35 * abs(profile[1])
                or final[1] * profile[1] < 0.0
            )
        )
        if not steering_clipped:
            # _auto_velocity_callback may already have stopped both velocity
            # components from the directional mask. Its zero profile is not a
            # recovered steering sample, so retain the confirmation timer.
            if "STEERING_SAFETY_STOP" not in self.profile_clamp_reasons:
                self.straight_steering_authority_lost_since = None
                self.straight_steering_authority_recovery_active = False
            return
        if self.straight_steering_authority_lost_since is None:
            ...
            return
        if (
            self.straight_steering_authority_recovery_active
            or now - self.straight_steering_authority_lost_since
            < self.straight_steering_authority_timeout
        ):
            return
        self._recover_lost_straight_steering(profile, final)

    def _recover_lost_straight_steering(
        self,
        profile_command: tuple[float, float, float],
        final_command: tuple[float, float, float] | None,
    ) -> None:
        """Stop, then relocate along the corridor before replanning."""
        if self.straight_steering_authority_recovery_active:
            return
        self.straight_steering_authority_recovery_active = True
        self.navigation_velocity.publish(Twist())
        active = self.active_segment
        pose = None if self.pose is None else dict(self.pose)
        self._nav_debug(
            "STRAIGHT_RECOVERY",
            reason="STEERING_AUTHORITY_LOST",
            action=(
                "RELOCATE_TO_STEERING_BAY"
                if active is not None and pose is not None
                else "REPLAN_FROM_CURRENT"
            ),
            profile_command=profile_command,
            final_command=final_command,
            safety_reason=self.safety_stop_reason,
            direction_mask=self.safety_direction_mask,
            destination_preserved=bool(self.paused_goal),
        )
        if active is not None and pose is not None:
            # Replanning at the same constrained pose selected essentially the
            # same line in the field logs. The existing bay worker proves a
            # forward/reverse relocation and the complete target turn against
            # both the Saved Map and fresh LiDAR before granting motion.
            self._start_turn_bay_recovery(
                pose,
                self.navigation_goal_generation,
                preferred_translation_direction=active.motion_direction,
                recovery_reason="STEERING_AUTHORITY_LOST",
                target_heading=active.fixed_heading,
            )
            return
        self._schedule_execution_replan(
            "STEERING_AUTHORITY_LOST", self.navigation_goal_generation
        )
    ...
    def _live_rotation_sweep_clear(self, now: float | None = None) -> bool:
        """Require a fresh full-body LiDAR sweep before any angular command."""
        timestamp = time.monotonic() if now is None else float(now)
        return bool(
    ...
    def _fail_sustained_sensor_time_pause(self, reason: str) -> None:
        ...
        self.latest_feedback.pop("terminal_reason", None)
        self.latest_feedback["recovery_reason"] = (
            f"SENSOR_TIME_SUSTAINED:{reason}"
        )
        self.latest_feedback["destination_preserved"] = bool(
            self.sensor_time_resume_context
        )
    ...
    def _resume_sensor_time_navigation_if_ready(self) -> None:
        ...
        with self.state_lock:
            ...
            if (
                ...
                or not self._critical_sensor_time_healthy()
                or now - self.last_sensor_time_resume_attempt_monotonic < 2.0
            ):
                return
            self.sensor_time_resume_in_progress = True
            self.last_sensor_time_resume_attempt_monotonic = now
        ...
        def resume() -> None:
            ...
            try:
                ...
                with self.state_lock:
                    ...
                    self._set_state(
                        "WAITING_FOR_DYNAMIC_CLEAR",
                        "localization_recovery_route_wait",
                    )
                    self.latest_feedback.pop("terminal_reason", None)
    ...
    def _stop_turn_planner_for_clearance(
        ...
        # Use conservative 2 mm bands. Rounding upward never weakens safety,
        # while keeping sub-millimetre covariance jitter from changing the
        # topology selected for an otherwise identical request.
        hard_margin = math.ceil(requested * 500.0 - 1e-9) / 500.0
        ...
        # Exact validation caches stay request-local, but topology stability
        # must span those planners because localization clearance is rebuilt
        # for every preview/replan request.
        planner._stable_primary_routes = base._stable_primary_routes
        return planner
    ...
    def _compute_path(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        def begin_automatic_plan_recovery(error: AdapterError) -> dict[str, Any]:
            """Keep an explicitly-started destination alive across no-route snapshots."""
            route_digest = hashlib.sha1(
            ...
            with self.state_lock:
                ...
                self.execution_goal = dict(resolved_goal)
        ...
        observed_front_keepout = self._live_front_keepout_for_route(
            [
                dict(start_pose),
                {
        ...
        presearch_front_keepout = bool(
            observed_front_keepout is not None
            and front_keepout_can_precede_route_search(live_start_directions)
        )
        live_front_keepout = (
            observed_front_keepout if presearch_front_keepout else None
        )
        dynamic_exclusions = (
            (live_front_keepout,)
        ...
        planning_exclusions = dynamic_exclusions
        if live_front_keepout is not None:
            self._nav_debug(
        ...
        elif observed_front_keepout is not None:
            self._nav_debug(
                "PLAN_LIVE_FRONT_KEEPOUT",
                center=(
                    round(observed_front_keepout[0], 3),
                    round(observed_front_keepout[1], 3),
                ),
                radius=round(observed_front_keepout[2], 3),
                front_clearance=self._finite_metric(
                    float(self.latest_corridor.front_clearance)
                ),
                action="DEFER_UNTIL_FIRST_SEGMENT",
                reason="REVERSE_ESCAPE_AVAILABLE",
            )
        ...
        if (
            ...
            # result has already occupied the complete 12-second request; an
            # immediate additional 15-second retry exceeds the edge adapter's
            # 20-second RPC timeout and is surfaced incorrectly as
            # NAVIGATION_ADAPTER_UNAVAILABLE even though ROS remains healthy.
        ...
        if (
            planner_result.route is None
            and planner_result.status == "NO_EXACT_STOP_TURN_ROUTE"
            and not live_obstacle_search
            and self.pose is not None
        ):
            # Exact feasibility includes the measured start pose. During the
            # failed field request AMCL refined that pose by 1.3 cm while the
            # first search ran, but the stale snapshot was returned as a hard
            # no-route result. Retry once only for a materially newer pose;
            # genuine static disconnections are not retried.
            fresh_start = dict(self.pose)
            if planning_pose_changed(
                start_pose,
                fresh_start,
                minimum_translation=(
                    self.stop_turn_exact_retry_minimum_translation
                ),
                minimum_yaw=self.stop_turn_exact_retry_minimum_yaw,
            ):
                fresh_directions = self._live_start_escape_directions(
                    fresh_start, resolved_goal
                )
                live_start_directions = fresh_directions
                live_start_clear = bool(fresh_directions)
                self._nav_debug(
                    "PLAN_SEARCH_RETRY",
                    first_status=planner_result.status,
                    first_expansions=planner_result.expansions,
                    ...
                    reason="FRESH_START_POSE",
                    old_start=start_pose,
                    new_start=fresh_start,
                    planning_time_budget=(
                        self.stop_turn_exact_retry_planning_budget
                    ),
                )
                start_pose = fresh_start
                request_planner = self._stop_turn_planner_for_clearance(
                    self._hard_route_side_clearance()
                )
                planner_result = request_planner.plan_result(
                    ...
                    allow_start_escape=True,
                    maximum_start_escape_distance=(
                        self.start_escape_max_distance
                    ),
                    maximum_start_escape_new_overlap_cells=1,
                    live_start_clear=bool(fresh_directions),
                    live_start_directions=fresh_directions,
        ...
        if planner_result.route is not None:
            ...
            if live_front_keepout is not None:
                ...
                planning_exclusions = (live_front_keepout,)
        ...
        if (
            ...
            and not planner_result.route.initial_reverse_clearance_recovery
            ...
            if bool(payload.get("auto_recover")):
                return begin_automatic_plan_recovery(error)
        ...
        try:
            ...
            if bool(payload.get("auto_recover")):
                return begin_automatic_plan_recovery(exc)
    ...
    def _navigate(
        ...
        *,
        ...
        initial_reverse_clearance_recovery = False
        if candidate is not None:
            ...
            initial_reverse_clearance_recovery = bool(
                candidate.get("initial_reverse_clearance_recovery", False)
            )
        ...
        with self.state_lock:
            if (
                ...
                raise AdapterError(
                    ...
                    "Navigation selection changed before motion could start",
                )
            self.navigation_goal_generation += 1
            if recovery_attempt:
                # Count only a validated route that is about to receive motion
                # authority. Planner-only retries are not physical attempts.
                self.navigation_recovery_attempts += 1
            else:
                self.navigation_recovery_attempts = 0
            self.latest_feedback = {
                "recoveries": self.navigation_recovery_attempts,
    ...
    def _start_escape_execution(
        ...
        *,
        ...
        escape = find_start_escape(
            ...
            # A fresh directional-safety permit plus an exact-clear endpoint
            # may bridge one isolated Saved Map raster disagreement. The
            # cumulative one-cell cap cannot authorize crossing a wall.
    ...
    def _complete_final_position_if_reached(
        ...
        def defer_completion(reason: str, **details: Any) -> bool:
            now = time.monotonic()
            if (
                now - self.last_goal_completion_deferred_log_monotonic
                >= 1.0
            ):
                self._nav_debug(
                    ...
                    reason=reason,
                    distance=distance,
                    position_tolerance=self.stop_turn_final_position_tolerance,
                    **details,
                )
                self.last_goal_completion_deferred_log_monotonic = now
            return False

        # The position-only tolerance is an early-arrival envelope, not
        # permission to finish beside the planned line.  A robot which stops
        # near the endpoint but several centimetres cross-track may leave its
        # rear corner in Saved Map structure; the next request then cannot
        # safely reverse to the turn bay it just passed.  Keep following the
        # final segment until it is inside the same geometric endpoint band
        # used by normal straight execution.
        segment_start = self.execution_points[-2]
        segment_end = self.execution_points[-1]
        if (
            self.active_segment is not None
            and self.active_segment.segment_index
            == len(self.execution_points) - 2
        ):
            segment_start = self.active_segment.effective_start
            segment_end = self.active_segment.endpoint
        try:
            progress = straight_segment_progress(
                segment_start,
                segment_end,
            ...
            return defer_completion("FINAL_SEGMENT_GEOMETRY_INVALID")
        if (
            abs(progress.signed_cross_track)
            > self.straight_endpoint_tolerance
        ):
            return defer_completion(
                "FINAL_CROSS_TRACK_OUTSIDE_ENDPOINT_BAND",
                cross_track=progress.signed_cross_track,
                cross_track_limit=self.straight_endpoint_tolerance,
                remaining_longitudinal=progress.remaining_longitudinal,
            )

        # Planning proves the endpoint footprint, but the measured pose may
        # enter the broad position tolerance before reaching that endpoint.
        # Never finalize a mission from a pose whose physical chassis already
        # overlaps occupied/unknown Saved Map raster.  Continuing the current
        # validated straight is safer than weakening start-escape on the next
        # mission to cross a new static cell.
        if self.saved_map is None:
            return defer_completion("SAVED_MAP_UNAVAILABLE")
        footprint_validation = self.saved_map.validate_footprint(
            float(pose["x"]),
            float(pose["y"]),
            ...
            padding=self.planning_footprint_padding,
            allow_unknown=False,
            code_prefix="GOAL_COMPLETION",
        )
        if not footprint_validation.valid:
            return defer_completion(
                "FINAL_FOOTPRINT_NOT_STATIC_VALID",
                validation_code=footprint_validation.code,
                cross_track=progress.signed_cross_track,
                remaining_longitudinal=progress.remaining_longitudinal,
            )
        return self._finish_execution_success(
            0.0,
    ...
    def _segment_execution_tick(self) -> None:
        ...
        shortest_direction = 1 if error >= 0.0 else -1
        if (
            direction != shortest_direction
            and not math.isclose(abs(error), math.pi, abs_tol=1e-6)
        ):
            # Defensive invariant: an angular command may never increase the
            # shortest signed heading error. Wait for that direction or move
            # to a new turn bay; never rotate almost a full revolution.
            self.execution_turn_direction = shortest_direction
            self.execution_phase = "WAIT_FOR_TURN_CLEAR"
            self.execution_turn_blocked_since = (
                self.execution_turn_blocked_since or now
            )
            ...
            self._nav_debug(
                "TURN_RECOVERY",
                action="REJECT_ERROR_INCREASING_DIRECTION",
                previous_direction=direction,
                shortest_direction=shortest_direction,
                heading_error=error,
                destination_preserved=bool(self.paused_goal),
            )
            return
    ...
    def _find_and_start_turn_bay(
        ...
        # During a multi-step narrow translation, retain the already-proven
        # forward/reverse direction. Re-ranking solely from the goal projection
        # after every chunk can flip the preference at the projection point and
        # make the chassis oscillate. If that direction becomes unsafe, the
        # fresh direction mask still removes it and permits the other escape.
        directions = live_directions
        if preferred_translation_direction in live_directions:
            directions = (
                int(preferred_translation_direction),
                *(
                    direction
                    for direction in live_directions
        ...
        with self.state_lock:
            ...
            # Edge transmits visualization only when this revision changes.
            # Publish the maneuver that is actually about to execute instead
            # of leaving the frontend on the superseded global route.
            self.visualization_revision += 1
    ...
    def _prepare_active_segment(
        ...
        if planned_progress.endpoint_distance <= self.straight_endpoint_tolerance:
            self._complete_active_segment(
                "ALREADY_AT_ENDPOINT",
                goal_generation,
                segment_token=self.execution_segment_token,
                segment_index=self.execution_segment_index,
                cancel_action=False,
            )
            return
        ...
        active_relocation = self.execution_relocation_reason in {
            "START_ESCAPE", "TURN_BAY",
        }
        if start_escape and self.saved_map is not None:
            ...
            endpoint = dict(refreshed_escape.end)
        ...
        try:
            live_validation = self._validate_executable_path(
                ...
                allow_monotonic_initial_overlap=active_relocation,
                maximum_new_initial_overlap_cells=(1 if start_escape else 0),
            )
        except AdapterError:
            self._schedule_execution_replan(
        ...
        self.active_segment = active
        self.execution_target_heading = active.fixed_heading
        # Re-anchoring deliberately changes the executed centerline to the
        # latest trusted pose. Keep the displayed remaining route under the
        # same authority as FollowPath, including after local recovery.
        displayed_execution = canonicalize_stop_turn_path([
            dict(active.effective_start),
            *(
                dict(point)
        ...
        if displayed_execution != self.latest_global_path:
            self.latest_global_path = displayed_execution
            self.visualization_revision += 1
        self._nav_debug(
            "SEGMENT_REANCHOR",
    ...
    def _complete_active_segment(
        ...
        *,
        ...
        # Geometric endpoint detection runs on the ROS timer while the Nav2
        # result arrives on an action callback thread. Both may observe the
        # same segment as current before either completes it. Claim the exact
        # generation/token/index transaction under one lock so completion is
        # idempotent and can advance the route at most once.
        with self.state_lock:
            active = self.active_segment
            completion_current = bool(
                goal_generation == self.navigation_goal_generation
                and self.current_state == "NAVIGATING"
                and self.execution_segment_token == segment_token
                and self.execution_segment_index == segment_index
                and (
                    active is None
                    or (
                        active.segment_token == segment_token
                        and active.segment_index == segment_index
                    )
                )
            )
            if not completion_current:
                current_token = self.execution_segment_token
                current_index = self.execution_segment_index
                current_generation = self.navigation_goal_generation
                current_state = self.current_state
                stale = True
                handle = None
                should_finish = False
                ...
                self.execution_final_turn = bool(
                    self.execution_segment_index
                    >= len(self.execution_points) - 1
                ...
                if not relocation_reason:
                    # This budget covers consecutive failed replans, not the
                    # whole mission. A completed normal segment proves useful
                    # progress and must not later masquerade as a dynamic
                    # obstacle cooldown.
                    self.execution_replan_attempts = 0
        ...
        if stale:
            self._nav_debug(
                "SEGMENT_COMPLETION_IGNORED",
                reason=reason,
                callback_generation=goal_generation,
                callback_segment_token=segment_token,
                callback_segment_index=segment_index,
                current_generation=current_generation,
                current_segment_token=current_token,
    ...
    def _finish_execution_success(
        ...
        *,
        ...
        completion_mode: str = "",
        allowed_states: tuple[str, ...] = ("NAVIGATING",),
    ) -> bool:
        ...
        with self.state_lock:
            ...
            completed_goal = (
                None
                if self.execution_goal is None
                else dict(self.execution_goal)
            )
            completed_pose = None if self.pose is None else dict(self.pose)
            completion_current = bool(
                goal_generation == self.navigation_goal_generation
                and self.current_state in allowed_states
            ...
            effective_position_tolerance = (
                ...
                else max(0.0, float(position_tolerance))
            )
            authoritative_distance = position_distance
            if completed_pose is not None and completed_goal is not None:
                authoritative_distance = math.hypot(
                    float(completed_pose["x"])
                    - float(completed_goal["x"]),
                    float(completed_pose["y"])
                    - float(completed_goal["y"]),
                )
            distance_valid = bool(
                authoritative_distance is not None
                and math.isfinite(float(authoritative_distance))
                and float(authoritative_distance)
                <= effective_position_tolerance
            )
            footprint_valid = False
            footprint_code = "POSE_OR_SAVED_MAP_UNAVAILABLE"
            if completed_pose is not None and self.saved_map is not None:
                footprint_validation = self.saved_map.validate_footprint(
                    float(completed_pose["x"]),
                    float(completed_pose["y"]),
                    float(completed_pose.get("yaw", 0.0)),
                    half_length=self.footprint_half_length,
                    half_width=self.footprint_half_width,
                    padding=self.planning_footprint_padding,
                    allow_unknown=False,
                ...
                footprint_valid = footprint_validation.valid
                footprint_code = footprint_validation.code
            if not completion_current:
                rejection_reason = "STALE_COMPLETION_TRANSACTION"
            elif not distance_valid:
                rejection_reason = "FINAL_POSITION_NOT_REACHED"
            elif not footprint_valid:
                rejection_reason = "FINAL_FOOTPRINT_NOT_STATIC_VALID"
            else:
                rejection_reason = ""

            if rejection_reason:
                handle = None
                physical_final_turn = False
            else:
                ...
                handle = self.current_goal_handle
                self.current_goal_handle = None
                # Invalidate every outstanding feedback/result callback as
                # part of the same terminal transaction.
                self.navigation_goal_generation += 1
                self.execution_segment_token += 1
                self.execution_final_turn = False
                self.execution_physical_final_turn = False
                ...
                self._set_state(
                    "SUCCEEDED",
                    (
                        "goal_reached_safe_standoff"
                        if completion_mode == "SAFE_STANDOFF"
                        else "goal_reached_by_stop_turn_segments"
                    ),
                )
                if completion_mode:
                    self.latest_feedback["arrival_mode"] = completion_mode
                self.paused_goal = None
        ...
        if rejection_reason:
            self._nav_debug(
                ...
                distance=authoritative_distance,
                position_tolerance=effective_position_tolerance,
                footprint_code=footprint_code,
                callback_generation=goal_generation,
                current_generation=self.navigation_goal_generation,
            )
            if rejection_reason != "STALE_COMPLETION_TRANSACTION":
                self._schedule_execution_replan(
                    rejection_reason, goal_generation
                )
            return False
    ...
    def _complete_blocked_goal_standoff(
        self,
        goal_generation: int,
        now: float,
    ) -> bool:
        """Finish near a destination only when that destination is occupied."""
        if (
            goal_generation != self.navigation_goal_generation
            ...
            or self.dynamic_blocked_keepout is None
            or self.dynamic_wait_started is None
            or not self.localized
            or self.localization_state != "READY"
            or (
                now - self.execution_pose_monotonic
                > self.execution_pose_freshness
            )
            or not self._corridor_sample_fresh_for_path(now)
        ):
            return False
        goal = dict(self.execution_goal or self.paused_goal or {})
        if "x" not in goal or "y" not in goal:
            return False
        pose = dict(self.pose)
        # Reproject the route-aligned return now. The latched keepout proves
        # repeated location, while this fresh projection prevents a different
        # later obstacle from completing against stale goal evidence.
        fresh_keepout = self._live_front_keepout_for_route(
            self.dynamic_blocked_route,
            self.dynamic_blocked_segment_directions,
            blocked_only=True,
        )
        if fresh_keepout is None:
            return False
        blocker_x, blocker_y, blocker_radius = fresh_keepout
        goal_distance = math.hypot(
            float(goal["x"]) - float(pose["x"]),
            float(goal["y"]) - float(pose["y"]),
        )
        blocker_goal_distance = math.hypot(
            float(goal["x"]) - blocker_x,
            float(goal["y"]) - blocker_y,
        )
        ...
        heading_error = self._yaw_delta(
            goal_heading, float(pose.get("yaw", goal_heading))
        )
        corridor_blocked, front_clearance = self._fresh_corridor_blockage(now)
        try:
            measured_linear = float(self.safety_snapshot.get("measured_linear"))
            measured_angular = float(self.safety_snapshot.get("measured_angular"))
        except (TypeError, ValueError):
            measured_linear = math.inf
            measured_angular = math.inf
        robot_stationary = bool(
            self.safety_snapshot.get("measured_velocity_fresh", False)
            and math.isfinite(measured_linear)
            and math.isfinite(measured_angular)
            and abs(measured_linear) <= 0.01
            and abs(measured_angular) <= 0.03
        )
        footprint_valid = bool(
            self.saved_map is not None
            and self.saved_map.validate_footprint(
                float(pose["x"]),
                float(pose["y"]),
                float(pose.get("yaw", 0.0)),
                half_length=self.footprint_half_length,
                ...
                padding=self.planning_footprint_padding,
                allow_unknown=False,
                code_prefix="GOAL_STANDOFF",
            ).valid
        )
        eligible = blocked_goal_standoff_is_safe(
            goal_distance=goal_distance,
            blocker_goal_distance=blocker_goal_distance,
            blocker_radius=blocker_radius,
            front_clearance=front_clearance,
            heading_error=heading_error,
            normal_goal_tolerance=self.stop_turn_final_position_tolerance,
            maximum_standoff_distance=self.blocked_goal_standoff_max_distance,
            minimum_front_clearance=(
                self.blocked_goal_standoff_minimum_clearance
            ),
            corridor_blockage_limit=(
                self.corridor_front_clearance
                + self.straight_endpoint_tolerance
            ),
            blocked_duration=max(0.0, now - self.dynamic_wait_started),
            confirmation_duration=self.blocked_goal_standoff_confirmation,
            safety_fresh=bool(
                corridor_blocked
                and self._atomic_safety_fresh(now)
                and self._critical_sensor_time_healthy()
                and self.safety_health.startswith("HEALTHY")
                and not self.estop_active
            ),
            robot_stationary=robot_stationary,
            current_footprint_valid=footprint_valid,
        )
        if not eligible:
            return False
        self._nav_debug(
            ...
            result="CONFIRMED",
            destination=goal,
            blocker_center=[round(blocker_x, 3), round(blocker_y, 3)],
            blocker_radius=round(blocker_radius, 3),
            blocker_goal_distance=round(blocker_goal_distance, 3),
            robot_goal_distance=round(goal_distance, 3),
            front_clearance=round(front_clearance, 3),
            blocked_seconds=round(now - self.dynamic_wait_started, 3),
        )
        return self._finish_execution_success(
            0.0,
            goal_generation=goal_generation,
            position_distance=goal_distance,
            cancel_active=True,
            position_tolerance=self.blocked_goal_standoff_max_distance,
    ...
    def _stop_unconfirmed_dynamic_recovery(self) -> None:
        """Keep an evidence-deadlocked mission waiting without fabricating data."""
        goal = dict(self.execution_goal or self.paused_goal or {})
        if goal:
        ...
        self.motion_owner = "NONE"
        self._set_state("WAITING_FOR_DYNAMIC_CLEAR", "blocker_position_unconfirmed")
        self.dynamic_wait_started = time.monotonic()
        self.navigation_velocity.publish(Twist())
        self._nav_debug(
            "DYNAMIC_OBSTACLE",
            action="WAIT_AND_RETRY_UNCONFIRMED",
    ...
    def _stop_persistent_moving_dynamic_recovery(self) -> None:
        """Renew a moving-obstacle wait without terminating its destination."""
        goal = dict(self.execution_goal or self.paused_goal or {})
        if goal:
            self.paused_goal = goal
        waited = (
        ...
        self.latest_feedback["destination_preserved"] = bool(goal)
        self.motion_owner = "NONE"
        self._set_state("WAITING_FOR_DYNAMIC_CLEAR", "moving_obstacle_has_priority")
        self.dynamic_wait_started = time.monotonic()
        self.navigation_velocity.publish(Twist())
        self._nav_debug(
            "DYNAMIC_OBSTACLE",
            action="WAIT_RENEWED_PERSISTENT_MOVING",
    ...
    def _dynamic_recovery_tick(self) -> None:
        ...
        if self.current_state not in {
            ...
            return
        self.navigation_velocity.publish(Twist())
        if awaiting_user_route:
            remaining = max(
                0.0,
                self.dynamic_route_selection_deadline_monotonic - now,
            )
            self.latest_feedback["route_selection_seconds_remaining"] = int(
                math.ceil(remaining)
            )
            if (
                self.dynamic_route_selection_deadline_monotonic > 0.0
                ...
                self.dynamic_route_selection_deadline_monotonic = 0.0
                self.dynamic_recovery_state = "AUTO_SELECT_RUNNING"
                self.latest_feedback["recovery_reason"] = (
                    "AUTO_SELECTING_SHORTEST_ROUTE"
                )
                generation = self.navigation_goal_generation
                threading.Thread(
                    target=self._auto_start_shortest_dynamic_route,
                    args=(generation,),
                    daemon=True,
                ).start()
            return
        ...
        dynamic_blocked = bool(
            ...
            or (
                ...
                and fresh
                and self._safety_blocks_turn(
                    self.execution_turn_direction or 1
                )
            )
        )
        ...
        replan_around = bool(
            persistent
            and (
                stationary_route_blocked
                or moving_route_blocked
                or controller_corridor_blocked
                # Planner/execution failures such as START_ESCAPE_UNAVAILABLE
                # have already proved that the accepted route cannot proceed.
                # They must get a bounded alternative search even when scan/map
                # disagreement prevents dynamic-object classification.
                or bool(self.dynamic_block_reason)
            )
        )
        if (
            ...
            # Stop immediately for a moving person, but only for the short
            # classification window. Once it expires, plan a route around it.
            self._set_state(
        ...
        if requires_alternative:
            # Use a real, repeatedly observed keepout when available. If scan
            # classification is unavailable, the live master costmap and exact
            # swept-footprint validator still gate every candidate; do not turn
            # missing classification into an infinite wait.
            self._observe_controller_blocker()
            # A persistent obstacle may occupy the semantic destination
            # itself. Once the stopped chassis is already inside the bounded
            # stand-off envelope, repeated routes to the same blocked point
            # cannot improve the outcome and must not loop forever.
            if self._complete_blocked_goal_standoff(
                self.navigation_goal_generation, now
            ):
                return
        ...
        self.latest_feedback["execution_phase"] = "DYNAMIC_REPLAN"
        self.latest_feedback["recovery_reason"] = (
            "SEARCHING_ALTERNATIVE_ROUTES"
            if requires_alternative
            else "VERIFYING_ORIGINAL_ROUTE"
        )
        generation = self.navigation_goal_generation
        threading.Thread(
    ...
    def _dynamic_route_relation(
        ...
        local_bypass = bool(
            ...
            >= self.dynamic_local_bypass_minimum_deviation
            and maximum_deviation
    ...
    def _present_dynamic_route_selection(
        ...
        """Expose global alternatives, then auto-start the shortest after 10 s."""
        if not candidates:
            return
        candidates.sort(
            key=lambda item: (
                float(item.get("total_length", math.inf)),
        ...
        deadline_unix_ms = int(
            (time.time() + self.dynamic_route_selection_timeout) * 1000
        )
        with self.state_lock:
            self.route_candidates = {
            ...
            self.latest_feedback["destination_preserved"] = True
            self.latest_feedback["route_selection_timeout_seconds"] = int(
                math.ceil(self.dynamic_route_selection_timeout)
            )
            self.latest_feedback["route_selection_seconds_remaining"] = int(
                math.ceil(self.dynamic_route_selection_timeout)
            )
            self.latest_feedback["route_selection_deadline_unix_ms"] = (
                deadline_unix_ms
            )
            self.dynamic_recovery_state = (
                "AWAITING_USER_ROUTE_CONFIRMATION"
            )
            self.dynamic_route_selection_deadline_monotonic = (
                now_monotonic + self.dynamic_route_selection_timeout
            )
            self.dynamic_replan_requires_alternative = False
            self.motion_owner = "NONE"
            self._set_state(
        ...
        self._nav_debug(
            ...
            auto_route_id=selected["route_id"],
            timeout_seconds=self.dynamic_route_selection_timeout,
            blocked_route_signature=self.dynamic_blocked_route_signature,
            destination=goal,
            destination_preserved=True,
    ...
    def _set_dynamic_blocked_no_route(
        self,
        reason: str,
        goal: dict[str, Any],
    ) -> None:
        """End a recovery decision explicitly instead of waiting forever."""
        with self.state_lock:
            if goal:
                self.paused_goal = dict(goal)
            self.dynamic_route_selection_deadline_monotonic = 0.0
            self.dynamic_recovery_state = "EXHAUSTED"
            self.execution_phase = "IDLE"
            self.motion_owner = "NONE"
            self.route_candidates = {}
            self.selected_route_id = ""
            self.latest_feedback["execution_phase"] = "BLOCKED"
            self.latest_feedback["recovery_reason"] = reason
            self.latest_feedback["destination_preserved"] = bool(goal)
            self.latest_feedback.pop("route_selection_deadline_unix_ms", None)
            self.latest_feedback.pop("route_selection_seconds_remaining", None)
            self._set_state("BLOCKED", reason.lower())
            self.visualization_revision += 1
        self.navigation_velocity.publish(Twist())
        if goal:
            self._persist_active_navigation_mission(
    ...
    def _auto_start_shortest_dynamic_route(
        self,
        ...
        if expected_generation != self.navigation_goal_generation:
            return
        ordered = sorted(
            self.route_candidates.values(),
            key=lambda item: (
                float(item.get("total_length", math.inf)),
                str(item.get("route_id") or ""),
            ),
        )
        goal = dict(self.paused_goal or self.execution_goal or {})
        for candidate in ordered:
            ...
            try:
                ...
                self._nav_debug(
                    "DYNAMIC_ROUTE_SELECTION",
                    action="AUTO_CANDIDATE_REJECTED",
                    route_id=route_id,
                    error=exc.code,
                )
                if expected_generation != self.navigation_goal_generation:
        ...
        if expected_generation != self.navigation_goal_generation:
            return
        self._set_dynamic_blocked_no_route(
            "ALL_ALTERNATIVE_ROUTES_INVALIDATED",
            goal,
        )
    ...
    def _attempt_dynamic_local_bypass_or_selection(
        ...
        try:
            ...
            self._set_dynamic_blocked_no_route(
                exc.code or "ALTERNATIVE_ROUTE_SEARCH_FAILED",
                goal,
            )
            return
        if expected_generation != self.navigation_goal_generation:
        ...
        self._nav_debug(
            "DYNAMIC_LOCAL_BYPASS",
            result="NO_SAFE_ROUTE",
            candidate_count=len(candidates),
            destination=goal,
            destination_preserved=True,
            autonomous_global_route_started=False,
        )
        # A route search from a chassis stopped only centimetres from the
        ...
        if (
            ...
            self.dynamic_recovery_state = "ACTIVE_RELOCATION"
            self.latest_feedback["execution_phase"] = "ACTIVE_RELOCATION"
            self.latest_feedback["recovery_reason"] = (
                "MOVING_TO_SAFE_BYPASS_POSITION"
            )
            self._nav_debug(
                "DYNAMIC_LOCAL_BYPASS",
                result="ACTIVE_RELOCATION_REQUESTED",
                attempt_count=self.dynamic_replan_attempt_count,
                preferred_action="REVERSE_IF_FRONT_BLOCKED",
                destination=goal,
                destination_preserved=True,
            )
            self._start_turn_bay_recovery(
                dict(self.pose), expected_generation
            )
            return
        self._set_dynamic_blocked_no_route(
    ...
    def _attempt_dynamic_replan(
        ...
        try:
            ...
            if (
                ...
                for alternative in alternatives:
                    ...
                    if (
                        ...
                        < self._hard_route_side_clearance()
                        and not alternative.initial_reverse_clearance_recovery
                    ):
                        self.dynamic_failed_route_signatures[
                    ...
                    self.pending_initial_reverse_clearance_recovery = bool(
                        alternative.initial_reverse_clearance_recovery
                    )
                    self._nav_debug(
                        "DYNAMIC_REPLAN",
            ...
            route_root = self.execution_route_id.split("-dynamic-", 1)[0]
            route_id = f"{route_root}-dynamic-{digest}"
            if self.pending_start_escape is not None:
                ...
                self.route_candidates[route_id] = {
                    ...
                    "initial_reverse_clearance_recovery": (
                        self.pending_initial_reverse_clearance_recovery
                    ),
                }
            self._nav_debug(
                "DYNAMIC_REPLAN",
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            ...
            if (
                ...
                if not localization_reliable:
                    self._localization_lost(
                        "SEGMENT_CONTROLLER_UNRELIABLE_POSE"
                    )
                else:
                    self._schedule_execution_replan(
                        "SEGMENT_CONTROLLER_FAILURE", goal_generation
                    )
        self.profile_limiter.reset()
        self.navigation_velocity.publish(Twist())
    ...
    def _set_recovery_terminal(
        ...
        """Convert exhausted recovery into a persistent, retryable wait."""
        with self.state_lock:
            if expected_generation != self.navigation_goal_generation:
                return
            goal = dict(self.execution_goal or self.paused_goal or {})
            ...
            self.latest_feedback["recovery_reason"] = reason
            self.latest_feedback["destination_preserved"] = bool(goal)
        self._enter_dynamic_wait(reason or "RECOVERY_RETRY")
        if goal:
            self._persist_active_navigation_mission(
                resume_automatically=True,
                goal=goal,
    ...
    def _plan_stop_turn_from_current(
        ...
        if (
            not result.route.initial_reverse_clearance_recovery
            and
        ...
        self.pending_initial_reverse_clearance_recovery = bool(
            result.route.initial_reverse_clearance_recovery
        )
    ...
    def _start_selected_route(
        ...
        *,
        ...
        with self.state_lock:
            if self.current_state != "ROUTE_SELECTION":
                raise AdapterError(
                    "STATE_CONFLICT",
                    "Route selection is no longer active",
                )
            ...
            candidate = self.route_candidates.get(route_id)
            goal = dict(self.paused_goal or {})
            dynamic_recovery_route = bool(
                candidate is not None
                and (
                    candidate.get("recovery_route_kind")
                    == "GLOBAL_ALTERNATIVE"
                    or self.route_selection_return_state
                    == "WAITING_FOR_DYNAMIC_CLEAR"
                )
            )
        if candidate is None or not goal:
        ...
        self._nav_debug("ROUTE_SELECTED", route_id=route_id, choice=choice)
        result = self._navigate(
            ...
            recovery_attempt=dynamic_recovery_route,
            expected_generation=selection_generation,
        )
        self.dynamic_route_selection_deadline_monotonic = 0.0
        if dynamic_recovery_route:
            self._reset_dynamic_recovery()
            self.execution_replan_attempts = 0
        self.route_candidates = {}
        self.selected_route_id = route_id
